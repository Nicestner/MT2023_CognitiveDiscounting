...
% !Takes about 15minutes to compute!
...
% Cogdis: Adjust Subplot-Grid!
    cogdis = [1, 0.85, 0.65, 0.45, 0.25, 0.05];
        ...
        if indeterminacy_error_occurred == false %Iterate through lambda only while there is no indeterminacy
            for i=1:length(lamb)          
                ...
                try
                    ...
                    if strcmp(ME.message, 'Blanchard & Kahn conditions are not satisfied: indeterminacy.')
...
title(han,'Indeterminacy Regions','FontSize', 17);
