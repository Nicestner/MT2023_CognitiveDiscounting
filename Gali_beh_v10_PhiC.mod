...
This version PhiC: 
- monetary policy rule that responds to consumption changes
...
the following parameters are defined in Matlab file galigabparms.mat before running:
    cogdisc, lambd, thet 
Use the script "setparms([M, lambda, theta])" first to define the parameters
*/
...
parameters M M_f beta delta mu_p alpha lambda theta varphi eta phi_pi phi_b phi_g rho rho_g gamma_g gamma_c lambda_p THETA_n THETA_t sigma gamma_i Gamma phi_c;
...
varphi = 0.2; 
eta = 1.0; 
phi_pi = 1.2; // MONPOL reaction to inflation
phi_c = 0.5;  // MONPOL reaction to aggregate consumption, Gali: 0
phi_b = 0.33; 
phi_g = 0.1; 
...
// POLICY: 
// 11 [name = 'Taylor Rule']
nom_rate = phi_pi*pi + phi_c*c + rho; //18
...
// stoch_simul(order=1,irf=20, nocorr, nodecomposition, nofunctions, nomoments, nograph)g tax y c invest n w b r_k pi k mu_hat_p nom_rate tobin_q ; // for Determinacy checks
stoch_simul(order=1,irf=20) g tax b y c invest n w pi k mu_hat_p nom_rate tobin_q r_k ; // for IRFs, additional variables: k mu_hat_p nom_rate tobin_q r_k
